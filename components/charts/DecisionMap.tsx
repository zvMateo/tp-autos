...
export function DecisionMap({ params }: DecisionMapProps) {
  ...
  const cells = useMemo(() => {
    ...
    for (let c = 0; c < COLS; c++) {
      ...
      for (let r = 0; r < ROWS; r++) {
        ...
        arr.push({
          x: M.left + (c / COLS) * plotW,
          y: M.top + (1 - (r + 1) / ROWS) * plotH,
          w: plotW / COLS + 0.6,
          h: plotH / ROWS + 0.6,
          k: best,
        });
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img"
      ...
      <text x={mx + 12} y={my - 8} fontSize="12" fontWeight="600" fill="var(--ink)">Martín</text>
    </svg>
  );
}
